...
def main():
    ...
    # 1. Construct model
    batching_policy = batching.BatchingPolicy()
    with batching_policy:
        model_batcher = registry.construct('model', config['model'],
    ...
    batch_executor = batching.Executor(root_module=model)
    for batch in train_data_loader:
        ...
        optimizer.zero_grad()
        with batching.BatchScheduler(batching_policy) as scheduler:
            losses = []
            for item in batch:
                losses.append(model_batcher.compute_loss_single(item))
        schedule = scheduler.serialize()
        results = batch_executor.run(schedule)
